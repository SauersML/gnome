...
<p>To control for simple memorization of training words (e.g., the model just learns to increase the probability of the words it must produce numbers for in the training set), I introduce an out-of-distribution (OOD) variant of the latent encoding evaluation, which uses a bunch of held-out words (358 concepts, no overlap on the training vocab). The same evaluation is performed.</p>
...
<p>This methodology finds no support for a hypothesis about anti-scheming training: Schoen et al. <span class="cite">[4]</span> notes that situational awareness and introspection are necessary precursors for covertly misaligned behavior, such as sandbagging or gradient hacking. Under that framework, increasing a model's capacity to reason about its own training process might increase the risk of scheming. We do not find support for or against this (likely only due to small model size and ineffective training).</p>

<p>Further, this work has takes the opposite perspective: that verifying self-reports against ground-truth internal signals acts as a form of honesty. Honesty is closely related to accurate self-prediction and reporting. It is an open question whether better self-prediction training increases scheming or honesty.</p>
...
const DEFAULT_PAGES: Page[] = [
	{
		...
		abstract: "We test whether Claude can recall hidden chain-of-thought strings from prior messages. Using sequence alignment and permutation-based null distributions, we find evidence for intermittent introspective access, including rare runs with one-in-a-million alignment scores. These findings are concordant with Anthropic's introspection research.",
